...
from ..domain.enums import PostFormat


class PostCreateSerializer(serializers.Serializer):
    post_format = serializers.ChoiceField(choices=PostFormat.choices(), required=True)
    text_content = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    ...
    previous_post_id = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    ...
    def validate(self, attrs):
        ...
        if not content_fields.get(post_format):
            raise serializers.ValidationError(f"For post_format '{post_format}', corresponding content is required.")

        for format, content in content_fields.items():
            if format != post_format and content is not None and content != "":
                raise serializers.ValidationError(f"Only content for '{post_format}' should be provided. Found content for '{format}'.")
        ...
        if post_format == PostFormat.VIDEO and video_content:
            allowed_video_types = ["video/mp4", "video/x-msvideo", "video/quicktime", "video/webm"]
