from ...users.application.dtos import CreateUserDTO, UserResponseDTO
from ...users.application.ports import UserRepositoryInterface
...
class RegisterRule:
    ...
    def execute(self, dto: CreateUserDTO) -> UserResponseDTO:
        self.password_service.validate(dto.password, dto.password_confirm)
...
class EmailOTPRequestRule:
    ...
    def execute(self, dto: EmailOTPRequestDTO) -> None:
        user = (
...
class VerifyEmailRule:
    ...
    def execute(self, dto: VerifyEmailDTO) -> UserResponseDTO:
...
class LoginRule:
    ...
    def execute(self, dto: LoginDTO) -> LoginResponseDTO:
        user = self.user_repository.find_by_email(dto.email, raw=True)
        if not user or not self.password_service.check(dto.password, user.password):
...
class ResetPasswordRule:
    ...
    def execute(self, dto: ResetPasswordDTO) -> None:
        otp_code = self.otp_code_repository.find_by_code(dto.otp_code)
        if not otp_code or otp_code.is_expired():
            raise ValueError("OTP code is invalid or expired.")
