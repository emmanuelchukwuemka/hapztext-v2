...
const VALID_TARGET_TYPES = ['post', 'user', 'message', 'call'];
const VALID_REASONS = ['spam', 'harassment', 'nudity', 'hate_speech', 'violence', 'other'];

// POST /reports — any authenticated user reporting a post, user, message, or
// call. Feeds the admin panel's moderation queue (routes/admin.js) — nothing
// here requires admin rights, this is the citizen side of that same table.
router.post('/', authMw, async (req, res) => {
  ...
  try {
    ...
    if (targetType === 'call') {
      await pool.query('UPDATE calls SET reports_count = reports_count + 1 WHERE id = $1', [targetId]).catch(() => {});
    }
